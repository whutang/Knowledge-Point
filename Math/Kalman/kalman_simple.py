...
for k in range(1, sz):
    # 预测过程
    x_predict[k] = x_update[k-1]  #X(k|k-1) = AX(k-1|k-1) + BU(k) + W(k),A=1,BU(k) = 0, w=0
    P_predict[k] = P_update[k-1] + Q  #P(k|k-1) = AP(k-1|k-1)A' + Q(k), A=1
    
    # 更新过程
    K[k] = P_predict[k] / ( P_predict[k] + R )  #Kg(k)=P(k|k-1)H'/[HP(k|k-1)H' + R],H=1
    x_update[k] = x_predict[k] + K[k] * (z[k] - x_predict[k])  #X(k|k) = X(k|k-1) + Kg(k)[Z(k) - HX(k|k-1)], H=1
    P_update[k] = (1 - K[k]) * P_predict[k]  #P(k|k) = (1 - Kg(k)H)P(k|k-1), H=1
